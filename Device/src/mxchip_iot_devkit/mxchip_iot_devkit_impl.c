...
{
    // Note: The only interface ID we're expecting to be quieried for is urn:contosoartshipping:position:1 so we don't bother checking the "id"
...
// infintely loop back and forth through simulated location
void Position_Telemetry_ReadLocation(POSITION_LOCATION * location)
{
    ...
    if(locationIdx == POSITION_ARR_LENGTH - 1) {
        direction = -1;
    } else if (locationIdx == 0 && direction == - 1) {
        direction = 1;
    }

    locationIdx = locationIdx + direction;
